...
function loadModel(f, inW, inH, outW, outH)
  ...
  modeltemp = torch.load(f)
  model2=nn.Sequential():add(modeltemp:get(1))
  model2=model2:cuda()
  model = nn.Sequential():add(modeltemp:get(2))
  model = model:cuda()
...
function backward(count, loss, data, gradients)
  ...
  if (storeCounter % storeInterval) == 0 then
    ...
    modelall=nn.Sequential()
    :add(model2)
    :add(model)
    torch.save(oFile .. storeCounter, modelall)
  end
