...
oFileRepro = 'obj_model_fcn_repro_VGG.net'
...
function constructModel(inW, inH, outW, outH)
  ...
  --attention
  modleparallel=nn.ConcatTable()
  modleparallel:add(
    nn.Sequential()
    :add(nn.SpatialConvolution(512, 512, 1, 1, 1, 1, 0, 0))
    :add(nn.ReLU())
    :add(nn.Sigmoid())
    -- :add(nn.Reshape(1024,4800))
    -- :add(nn.SoftMax())
    -- :add(nn.Reshape(1,1024,60,80))
  )
  modleparallel:add(nn.Identity())

  model:add(modleparallel)
